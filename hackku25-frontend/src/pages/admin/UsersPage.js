...
const UsersPage = () => {
  ...
  const data = useMemo(
    () =>
      users.map((user) => ({
        ...
        email: user.email,
        role: user.role,
        created_on: <FormattedDate utcDateString={user.created_on} />, // Use the new component
      })),
    [users]
  );
  ...
  const columns = useMemo(
    () => [
      { Header: "User ID", accessor: "id" }, // Add User ID column
      ...
      {
        Header: "Actions",
        accessor: "action", // Use a different accessor for actions
        Cell: ({ row }) => (
          <Button
            color="red"
            size="xs"
